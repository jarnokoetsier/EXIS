...
#1. Source run the global R script every time you use the app.
...
#4. Run the below-written code. In RStudio, you can simply press "Run App" in the top-right corner.
...
#INSTALL AND LOAD SHINY PACKAGES
...
server <- function(input, output, session){
  ...
  select.top.table <- eventReactive(input$diffexpr.ok,{
    ...
    if (brainarray() == "transcript"){
      select.top.table <- transcript_selection(top.table, 
                                               genes = genes(), 
    ...
    if (brainarray() == "exon"){
      select.top.table <- exon_selection(top.table, 
                                          annotated = annotated(), 
                                          genes = genes(), 
                                          transcripts = transcripts(), 
  ...
  output$exprboxplot <- renderPlot({
    if (brainarray() == "exon"){
      if (length(exonboxplot()) > 0){
        if (exonboxplot() == TRUE){
          plot <- makeBoxplots(contrast = table.choice(), 
                       meta = meta(), 
                       data.expr = data.expr(), 
                       annotated = annotated(), 
                       gene = genes(), 
