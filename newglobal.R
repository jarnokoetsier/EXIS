...
pca.plot <- function(data.PC, meta, hpc = 1, vpc = 2, interactive = TRUE) {
...
transcript_selection <- function(top.table, gene = NULL, transcripts = NULL) {
  ...
  if (is.null(gene) & is.null(transcripts)){
...
exon_selection1 <- function(top.table, annotated, gene = NULL, transcripts = NULL, unique_exons = TRUE, genome_wide = FALSE) {
...
makeBoxplots <- function(contrast, meta, data.expr, gene = NULL, transcripts = NULL, unique_exons = TRUE, genome_wide = FALSE) {
  ...
  data.expr1 <- inner_join(data.expr1, as.data.frame(annotation1$Probe.set), by = c("Probe.set" = "annotation1$Probe.set"))
...
probemapping.ense <- function(chiptype, organism, version, gene, annotated){
  ...
  probes_remainder <- inner_join(gene_probes_all, probefreq_remainder, by = c("probe" = "Var1"))
  probes_remainder <- cbind(probes_remainder, rep("No", nrow(probes_remainder)))
  colnames(probes_remainder) <- c("Probe.set", "Exon.group","Exon","Transcript", "Gene", "Probe", "Chr", "Chr.Strand", "Chr.From", "Freq", "Unique.probe")
  
  total <- rbind(probes_filtered, probes_remainder)
  number <- (4*length(unique(total$Exon.group)) + length(unique(total$Probe)))/5
  
  #Make plot
  gg <- ggplot(total, aes(y = Exon.group, x = Probe, fill = Unique.probe, text = paste(" Chr:", Chr, "<br>","Chr.Strand:", Chr.Strand, "<br>", "Chr.From:", Chr.From))) +
    geom_point(size = ((5*14)/number), shape = 22, color = "black") +
    theme_light() +
    theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
    ggtitle("Probe mapping") +
